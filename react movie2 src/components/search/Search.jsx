...
const renderTitle = (title) => (
  <span>
    ...
    <a
      ...
      href={`http://localhost:3000/${title}`}
      target="_blank"
...
export default function Search() {
  ...
  function handleLoadMovie(id) {
    ...
    switch (data.media_type) {
      case "movie":
        return history.push(`/movie/${data.id}`);

      case "tv":
        return history.push(`/tv/${data.id}}`);
      case "person":
        return history.push(`/selebs/${data.id}`);
    }
  }
